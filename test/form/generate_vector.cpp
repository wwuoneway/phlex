...
{
  public:
    GaussianGenerator(int const n_time_ticks, int const seed, float const mean, float const stddev): m_n_time_ticks(n_time_ticks), m_gen(seed), m_dist(mean, stddev) {}

    std::vector<int> operator ()([[maybe_unused]] data_cell_index const& idx)
    {
      std::vector<int> randoms(m_n_time_ticks);
      for(auto& random: randoms) random = m_dist(m_gen);
      return randoms;
    }
...
{
  int const seed = config.get<int>("seed", std::chrono::system_clock::now().time_since_epoch().count());
...
PHLEX_REGISTER_ALGORITHMS(graph, config)
{
  ...
  graph.transform("add_wires", [](std::vector<int> const& lhs, std::vector<int> const& rhs)
                  {
